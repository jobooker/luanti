...
TRACE_SHA = "5b4ac557d52ad5457f74e2b93514fddf4b34446049ff1b0cfbca785464cefa46"
...
def build_nodescend(src):
    ...
    return (HDR % ("claude_trace/opengl_fragment.glsl", sha(TRACE),
                   "DESCENT COMPILED OUT: descendCell() and both call "
                   "sites deleted.")) + t


# --- the counters variant --------------------------------------------
# Globals, declared just above descendCell() so both walks can reach
# them. They are plain adds in the inner loops; that makes this shader
# measurably SLOWER than the one it counts for, which is why no timing
# number is ever taken from it.
COUNTER_GLOBALS = """
// --- STEP COUNTERS (measurement variant only) ------------------------
// g_fine   1/16 m steps taken inside descendCell(), all rays, all depths
// g_coarse 1 m steps taken in march()'s outer DDA, all rays, all depths
...
def build_counters(src):
    t = src
    # globals + pack16, in front of descendCell()
    t = replace_once(
        t,
        "bool descendCell(vec3 cell, vec3 ro, vec3 rd, float tEnter,",
        COUNTER_GLOBALS
        + "bool descendCell(vec3 cell, vec3 ro, vec3 rd, float tEnter,",
        "counter globals")
    # the fine (1/16 m) loop
    t = replace_once(
        t,
    ...
    t = replace_once(
        ...
        "\tfor (int i = 0; i < MARCH_STEPS; i++) {\n\t\tg_coarse += 1.0;\n",
        "coarse step counter")
    # both descend call sites set the "this ray descended" flag
    t = replace_once(
        t,
        "\t\tif (s0.a > CLASS_SUBVOX_LO && s0.a < CLASS_SUBVOX_HI) {\n",
        "\t\tif (s0.a > CLASS_SUBVOX_LO && s0.a < CLASS_SUBVOX_HI) {\n"
        "\t\t\tg_desc = 1.0;\n",
        "descend flag, starting cell")
    t = replace_once(
        t,
        "\t\t\tif (!descendCell(cell, ro, rd, t, n, false, sn, st))\n",
        "\t\t\tg_desc = 1.0;\n"
        "\t\t\tif (!descendCell(cell, ro, rd, t, n, false, sn, st))\n",
        "descend flag, in-loop")

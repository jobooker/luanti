...
// DEAD WEIGHT K=16 (fetch): inert, unreachable, un-foldable. Timing arm only.
// =====================================================================
		...
		out vec3 le, out float tHit, out vec3 cellOut)
{
	...
	for (int i = 0; i < WALK_STEPS; i++) {
		...
		// DEAD WEIGHT, K = 16 (texture reads only, minimal live registers). Never executed: game.cpp
		...
		if (claudeDescend > 5.0) {
			vec4 w = g_dw;
			w = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w.xyz) / GRID_S);
			g_dw += w;
			w = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w.xyz) / GRID_S);
			g_dw += w;

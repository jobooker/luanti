...
// DEAD WEIGHT K=32 (both): inert, unreachable, un-foldable. Timing arm only.
// =====================================================================
		...
		out vec3 le, out float tHit, out vec3 cellOut)
{
	...
	for (int i = 0; i < WALK_STEPS; i++) {
		...
		// DEAD WEIGHT, K = 32 (registers + texture reads). Never executed: game.cpp
		// clamps claude_descend to [0,1] and this asks for > 5.
		// It cannot be folded away (claudeDescend is a uniform) and
		// it cannot be dead-stripped (g_dw reaches gl_FragColor).
		if (claudeDescend > 5.0) {
			vec4 w0 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + g_dw.xyz) / GRID_S);
			vec4 w1 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w0.xyz) / GRID_S);
			vec4 w2 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w1.xyz) / GRID_S);
			vec4 w3 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w2.xyz) / GRID_S);
			vec4 w4 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w3.xyz) / GRID_S);
			vec4 w5 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w4.xyz) / GRID_S);
			vec4 w6 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w5.xyz) / GRID_S);
			vec4 w7 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w6.xyz) / GRID_S);
			vec4 w8 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w7.xyz) / GRID_S);
			vec4 w9 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w8.xyz) / GRID_S);
			vec4 w10 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w9.xyz) / GRID_S);
			vec4 w11 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w10.xyz) / GRID_S);
			vec4 w12 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w11.xyz) / GRID_S);
			vec4 w13 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w12.xyz) / GRID_S);
			vec4 w14 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w13.xyz) / GRID_S);
			vec4 w15 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w14.xyz) / GRID_S);
			vec4 w16 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w15.xyz) / GRID_S);
			vec4 w17 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w16.xyz) / GRID_S);
			vec4 w18 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w17.xyz) / GRID_S);
			vec4 w19 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w18.xyz) / GRID_S);
			vec4 w20 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w19.xyz) / GRID_S);
			vec4 w21 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w20.xyz) / GRID_S);
			vec4 w22 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w21.xyz) / GRID_S);
			vec4 w23 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w22.xyz) / GRID_S);
			vec4 w24 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w23.xyz) / GRID_S);
			vec4 w25 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w24.xyz) / GRID_S);
			vec4 w26 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w25.xyz) / GRID_S);
			vec4 w27 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w26.xyz) / GRID_S);
			vec4 w28 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w27.xyz) / GRID_S);
			vec4 w29 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w28.xyz) / GRID_S);
			vec4 w30 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w29.xyz) / GRID_S);
			vec4 w31 = texture3D(claudeTraceGrid,
					(ci + vec3(0.5) + w30.xyz) / GRID_S);
			g_dw += w0 + w1 + w2 + w3
					+ w4 + w5 + w6 + w7
					+ w8 + w9 + w10 + w11
					+ w12 + w13 + w14 + w15
...
void main(void)
{
	...
	// + g_dw.rgb: always exactly zero at run time (see the

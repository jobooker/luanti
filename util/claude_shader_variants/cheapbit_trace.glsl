...
// subvoxSolid() BIT EXTRACTION REMOVED -- fetch kept, maths dropped. Cost arm only; the image is wrong.
// =====================================================================
...
bool subvoxSolid(vec3 rc, vec3 sc)
{
	vec3 texel = vec3(rc.x * 2.0 + floor(sc.x / 8.0),
			rc.y * SUBV + sc.y,
			rc.z * SUBV + sc.z);
	// CHEAP-BIT CONTROL (measurement variant, lean-descend 2026-08-17).
